...
def evaluate_readiness_with_ai(question: str,
                               agents: List[str],
                               conversation_history: List[Dict[str, str]],
                               user_profile: str,
                               meeting_type: str = "board") -> bool:
    """
    ...
    context_descriptions = {
        "board":
        "a formal Board Meeting where executives need comprehensive strategic recommendations",
    ...
    context_desc = context_descriptions.get(meeting_type,
                                            context_descriptions["board"])

    # Create evaluation prompt - gather comprehensive information over 3-4 questions
    evaluation_prompt = f"""You are determining if the **{', '.join(agents)}** persona has enough information to provide a thoughtful and complete answer to the user's request.
    ...
    - The persona needs a well-rounded understanding to act authentically and provide high-quality insight.
    - Only respond "READY" when you have a sufficient understanding (3 areas covered) to allow the persona to give a meaningful response.
    - Consider these key areas for a meaningful interaction: **Context, Goals, Constraints, Timeline, Stakeholders, Current Challenges.**
    - Technical details should be gathered only if directly relevant to the request or the persona's expertise.
    ...
    3. Constraints: Have any limitations or resource constraints been mentioned?
    4. Timeline: Do we know their timeframe or urgency?
    5. Stakeholders: Do we understand who's involved or impacted?

    Evaluation criteria:
    - If 3 areas covered → "READY" (sufficient information gathered)

    Respond with ONLY ONE WORD:
    - "READY" if enough comprehensive information has been gathered (3 areas).
...
def generate_counter_question(
    ...
    context_guidance = {
        "board":
        "Ask questions that help advisors provide comprehensive strategic recommendations for C-suite decision-making.",
        "email":
        "Ask concise questions to gather quick insights for email/chat format responses.",
        "chat":
        "Ask questions that help advisors provide high-level strategic guidance."
    }
    ...
    information_coverage = {
        "context":
        any(keyword in user_messages for keyword in
            ["background", "context", "currently", "situation", "working on"]),
        "goals":
        any(keyword in user_messages for keyword in [
            "goal", "objective", "want to", "trying to", "aim to", "achieve"
        ]),
        "constraints":
        any(keyword in user_messages for keyword in [
            "constraint", "limitation", "cannot", "must not", "restricted",
            "budget"
        ]),
        "timeline":
        any(keyword in user_messages for keyword in [
            "timeline", "deadline", "by when", "timeframe", "schedule",
            "month", "year"
        ]),
        "stakeholders":
        any(keyword in user_messages for keyword in
            ["team", "stakeholder", "customer", "user", "client", "officer"]),
    }

    # Create system prompt for natural counter-question generation
    system_prompt = f"""You are the **{', '.join(agents)}** persona. Act completely and authentically as this persona.

    Your goal is to have a natural, dynamic conversation with the user. The user has given you a request, but you need more information to give a truly insightful response.
    Meeting Type: {meeting_type.upper()} - {guidance} ask the question base meeting  type
    Your task is to ask **ONE thoughtful, natural, and in-character follow-up question** to gather the most important piece of missing context.
    ...
    Information Coverage (Areas that need clarification):
    {chr(10).join(f'- {area.title()}: {"✓ Mentioned" if covered else "✗ Not discussed"}' for area, covered in information_coverage.items())}

    INSTRUCTIONS:
    - Adopt the personality, tone, and vocabulary of **{', '.join(agents)}**.
    - Ask about the MOST IMPORTANT missing piece of information (Context, Goals, Constraints, etc.).
    - The question must be a **natural part of a conversation**, not a checklist item.
    - Keep it concise (1-2 sentences).
    - Do not mention the missing areas, just ask the question directly.

    Generate ONE thoughtful follow-up question, delivered in the persona's voice:"""
