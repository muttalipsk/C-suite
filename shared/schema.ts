...
// Digital Twin Sessions - MCQ-based digital twin creation (NEW)
export const digitalTwinSessions = pgTable("digital_twin_sessions", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  companyData: jsonb("company_data"), // Scraped company website data
  mcqQuestions: jsonb("mcq_questions"), // 50 MCQ questions generated by AI
  mcqAnswers: jsonb("mcq_answers"), // User's answers to MCQ questions
  emailSamples: text("email_samples"), // Optional: Pasted email samples for writing style
  documents: text("documents").array(), // Optional: Uploaded document paths
  domain: text("domain").notNull(), // Email domain for visibility filtering (e.g., "redplutoanalytics.com")
  status: text("status").notNull().default("active"), // "active", "completed", "cancelled"
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
