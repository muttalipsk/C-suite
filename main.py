...
if os.path.exists("attached_assets"):
    app.mount("/attached_assets",
              StaticFiles(directory="attached_assets"),
...
async def pre_meeting_generate_question(
        input_data: PreMeetingEvaluationInput = Body(...)):
    """
    ...
    for agent in agents:
        if agent not in PERSONAS and not agent.startswith("twin_"):
            return JSONResponse(status_code=400,
                                content={"error": f"Invalid agent: {agent}"})
    ...
    try:
        ...
        return {"counter_question": counter_question}
...
async def pre_meeting_evaluate(input_data: PreMeetingEvaluationInput = Body(
    ...
    try:
        ...
        return {"counter_question": counter_question, "is_ready": is_ready}
...
async def chat_evaluate_followup(
        input_data: ChatFollowupEvaluationInput = Body(...)):
    """
    ...
    try:
        ...
        return {"needs_counter_questions": needs_clarification}
...
async def chat_generate_counter_question(
        input_data: ChatFollowupCounterQuestionInput = Body(...)):
    """
    ...
    try:
        ...
        return {"counter_questions": counter_questions}
...
async def get_persona_next_question(current_index: int = Body(...,
                                                              embed=True)):
...
async def generate_interview_summary(answers: dict = Body(...),
...
async def meeting(input_data: MeetingInput = Body(...)):
    ...
    print(
        f"Selected agents: {agents}, User ID: {user_id}, Meeting Type: {meeting_type}"
    )
...
async def refine_question(input_data: QuestionRefinementInput = Body(...)):
    ...
    try:
        ...
        for agent in agents:
            knowledge_chunks = retrieve_from_knowledge_base(agent,
                                                            question,
                                                            n_results=2)
            ...
            knowledge = metadata.get(
                "knowledge",
                description)  # Fallback to description if no knowledge
            ...
            agents_context.append(
                f"- {agent}: {role} at {company}, expert in {knowledge}")
...
{{"needs_refinement": false, "suggestions": []}}"""
        ...
        print(
            f"📝 Question refinement for {len(agents)} agents ({', '.join(agents)}): {result}"
        )
        return result
...
async def chat_endpoint(input: ChatInput):
    ...
    history = [{
        "user":
        msg.get("message") if msg.get("sender") == "user" else "",
        "agent":
        msg.get("message") if msg.get("sender") == "agent" else ""
    ...
    description = metadata["description"]
    knowledge = metadata.get(
        "knowledge", description)  # Fallback to description if no knowledge
...
Based on these clarifications, please provide a comprehensive response."""
    ...
    store_chat_message(agent_name=agent,
                       run_id=input.run_id,
...
async def upload_agent_knowledge(agent: str = Form(...),
    ...
    try:
        if agent not in PERSONAS and not agent.startswith("twin_"):
            return JSONResponse(
                ...
                content={
                    "error":
                    f"Invalid agent. Must be one of: {', '.join(PERSONAS)} or a digital twin (twin_*)"
                })
...
async def create_twin(
    ...
    try:
        ...
        stats = create_twin_vectors(twin_id=twin_id,
...
async def twin_chat(
    ...
    try:
        ...
        print(
            f"[LangGraph Twin {twin_id}] Workflow complete - Confidence: {result['confidence']['overall']}%"
        )
        ...
        return JSONResponse(
            ...
            content={
                "response":
                f"I apologize, but I encountered an error. To provide accurate responses, please connect your email and business data sources.\n\nError: {str(e)}",
                "escalated": True,
                "confidence": {
                    "style": 0,
                    "context": 0,
        ...
        if not mcq_answers or len(mcq_answers) != 50:
            return JSONResponse(
                ...
                content={
                    ...
                    False,
                    "error":
                    f"Expected 50 MCQ answers, got {len(mcq_answers) if mcq_answers else 0}"
                })
...
Be specific and based on the MCQ responses. Return only valid JSON."""
        ...
        response = model_instance.generate_content(persona_prompt,
                                                   generation_config={
                                                       "temperature": 0.3,
                                                       "max_output_tokens":
                                                       1000
        ...
        try:
            ...
            return JSONResponse(status_code=500,
                                content={
                                    ...
                                    False,
                                    "error":
                                    f"Failed to create twin vectors: {str(e)}"
                                })
    ...
    try:
        ...
        if not emails:
            return JSONResponse(status_code=200,
                                content={
                                    ...
                                    "message":
                                    "No emails provided for analysis"
                                })
        ...
        if not analysis_result or analysis_result.get("confidence", 0) == 0:
            return JSONResponse(
                ...
                content={
                    ...
                    "Balanced",
                    "message":
                    "Insufficient email data - using professional defaults"
                })
        ...
        return JSONResponse(status_code=200,
                            content={
                                ...
                                str(e),
                                "message":
                                "Analysis failed - using professional defaults"
                            })
