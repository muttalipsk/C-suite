...
export function ConversationHistory({ onSelectConversation, onLoadChat, selectedConversation, allAgents }: ConversationHistoryProps) {
  ...
  return (
    <div className={`border-l bg-sidebar h-screen flex flex-col transition-all duration-300 ${isExpanded ? 'w-80' : 'w-16'}`}>
      ...
      {isExpanded && (
        <ScrollArea className="flex-1 overflow-y-auto [&>div>div[style]]:!block [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
